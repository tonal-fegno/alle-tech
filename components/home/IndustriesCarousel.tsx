...
export default function IndustriesCarousel({ industries }: { industries: Industry[] }) {
  const trackRef = useRef<HTMLDivElement>(null);
  const [progress, setProgress] = useState(0);
  const [cycle, setCycle] = useState(0);

  const setWidth = industries.length * CARD_STEP;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      settleTimeout = setTimeout(() => {
        ...
        if (el.scrollLeft < setWidth - CARD_STEP / 2) {
          el.scrollLeft += setWidth;
        } else if (el.scrollLeft > max - setWidth + CARD_STEP / 2) {
          el.scrollLeft -= setWidth;
        }
      }, 120);
    };
  ...
  }, [cycle, scrollByCards, industries.length]);
