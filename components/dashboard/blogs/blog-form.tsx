...
import { blogSchema, type BlogFormValues } from "@/lib/zod-schemas";
import {
  Form,
  FormControl,
  FormField,
  ...
  return (
    <Form {...form}>
      <form id="blog-form" onSubmit={form.handleSubmit(onSubmit)}>
        <FormShell
          title={blogId ? `Edit blog${defaultValues?.title ? `: ${defaultValues.title}` : ""}` : "New blog"}
          backHref="/dashboard/blogs"
          formId="blog-form"
          sections={SECTIONS}
          isSubmitting={form.formState.isSubmitting}
          submitLabel={blogId ? "Save changes" : "Create blog"}
        >
          <FormSection id="details" title="Details" description="Listing card info and metadata." icon={Info}>
            ...
            <FormField control={form.control} name="image" render={({ field }) => (
              <FormItem><FormLabel>Image</FormLabel><FormControl><ImageUpload className="max-w-sm" prefix="blogs" value={field.value} onChange={field.onChange} /></FormControl><FormMessage /></FormItem>
            )} />
          </FormSection>
