...
import { ArrowLeft, ChevronRight, Check, Zap, CheckCircle2 } from "lucide-react";
...
// Used only when a solution has no whyAlleTechItems of its own yet.
...
export default function SolutionDetailLayout({
  ...
  // If this solution hasn't had its detail sections filled in yet (via the
  // dashboard), render a simple fallback layout driven by the base fields.
  if (!solution.heroTitle) {
    return (
  ...
  const whyAlleTechItems =
    solution.whyAlleTechItems.length > 0
      ? solution.whyAlleTechItems
      : defaultWhyAlleTechItems;
  ...
  return (
    <div className="text-ink antialiased">
      ...
      >
        {solution.heroImage &&
          (solution.heroImage.startsWith("http") ||
            solution.heroImage.startsWith("/")) ? (
          <motion.div
        ...
        <div className="relative z-10 max-w-4xl mx-auto w-full px-6 flex flex-col items-start text-left">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-semibold tracking-tight text-white mb-1 leading-tight max-w-4xl flex flex-wrap">
            {solution.heroTitle.split(" ").map((word, idx) => (
              <motion.span
                key={idx}
                initial={{ opacity: 0, y: 4, filter: "blur(4px)" }}
                animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
            ...
            {solution.challenges.map((ch, idx) => (
              ...
              >
                ...
                <h3 className="text-base font-semibold text-ink mb-4 flex items-center gap-3">
                  <DynamicIcon name={ch.icon} className="w-[22px] h-[22px] text-ink shrink-0" />
                  <span>{ch.title}</span>
                </h3>
                {/* Card Description */}
              ...
              {solution.outcomes.map((out, idx) => (
                ...
                >
                  ...
                  <div className="w-12 h-12 rounded-2xl bg-white text-[#2C8FCE] flex items-center justify-center shrink-0 mb-6 shadow-sm border border-neutral-100 group-hover:scale-105 transition-transform duration-300">
                    <DynamicIcon name={out.icon} className="w-[22px] h-[22px]" />
                  </div>
        ...
        <section className="bg-bg-3 border-y border-border-gray/20 section-padding">
          <div className="container-main">
            <div className="text-center mb-16">
              ...
              <h2 className="heading-2">
                {solution.relatedIndustriesTitle}
              </h2>
            </div>
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {solution.integrations.map((integ, idx) => (
                ...
                >
                  ...
                  >
                    <DynamicIcon name={integ.icon} className="size-5 text-[#2C8FCE]" />
                  </div>
                  <div>
